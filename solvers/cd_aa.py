...
class Solver(BaseSolver):
    ...
    def set_objective(self, X, y, lmbd, fit_intercept):
        ...
        solver_enet(X, y, lmbd, max_iter=1)

    def run(self, n_iter):
        if n_iter == 0:
            # increase size by 1 to consider ws_size
            self.coef = np.zeros(self.X.shape[1])
        else:
            w = solver_enet(self.X, self.y, alpha=self.lmbd,
...
######################
######################
# based on anderoncd repo to reproduce exp in paper
# Anderson acceleration of coordinate descent
# commit sha: 88f888ee7698f5507dc1c833337a83c42e37ff15
# url: https://github.com/mathurinm/andersoncd/tree/88f888ee7698f5507dc1c833337a83c42e37ff15
######################
######################
...
def solver_enet(
    ...
    if algo == 'pgd' or algo == 'fista':
        if is_sparse:
            pass  # L = power_method(X, max_iter=1000) ** 2
        else:
            L = norm(X, ord=2) ** 2
